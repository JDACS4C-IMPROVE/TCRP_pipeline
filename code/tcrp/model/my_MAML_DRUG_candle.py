...
class tcrp_candel(candle.Benchmark):
    def set_locals(self):
...
###############################################################################################
def zero_shot_test(unseen_test_loader, feature_dim, layer, hidden, observed_tissue_model):
...
def unseen_tissue_learn(unseen_train_loader, unseen_test_loader, feature_dim, layer, hidden, inner_lr,
                        num_inner_updates, observed_tissue_model):
    unseen_tissue_model = mlp(feature_dim, layer, hidden)
...
def meta_update(test_loader, ls, observed_tissue_model, observed_opt):
    # print 'Meta update'
...
def load_data(drug, tissue, K):
    ...
    train_feature, train_label, tissue_index_list, drug_test_feature, drug_test_label, _ = load_data_cell_line(
        drug_tissue_map, drug, tissue, K, path=data_dic + "/")
    feature_dim = train_feature.shape[1]
    return train_feature, train_label, tissue_index_list, drug_test_feature, drug_test_label, feature_dim


def build_model(feature_dim, layer, hidden, meta_lr):
    ...
    return observed_tissue_model, observed_opt


############################################
# load testing dataset to validate the training loss
def load_unseen_data(drug, tissue, num_trials, K):
    ...
    for trial in range(num_trials):
        ...
        for k in range(1, K + 1):
            ...
            train_data = np.load(
                os.path.join(test_data_path, '{}_{}_{}-shot_{}-trial_train.npz'.format(drug, tissue, k, trial)))
    ...
    return unseen_train_loader_list, unseen_test_loader_list


def load_zero_shot_data_list(drug_test_feature, drug_test_label):
    ...
    return zero_test_data_list


def make_output_dir(drug, tissue, meta_lr, inner_lr, layer, tissue_num):
    ############################################
    hyperparam_str = str(meta_lr) + '_' + str(inner_lr) + '_' + str(layer) + '_' + str(tissue_num)
    output_folder = os.path.join(project_root, "results", "predictions", drug, tissue, hyperparam_str)
    mkdir_cmd = 'mkdir -p ' + output_folder
    os.system(mkdir_cmd)
    return hyperparam_str, output_folder


#
def train(inner_net, train_feature, train_label, output_folder, num_updates, meta_batch_size, inner_batch_size, layer,
          hidden,
          zero_test_data_list, unseen_train_loader_list, unseen_test_loader_list,
          inner_lr, num_inner_updates, tissue_num, patience,
          feature_dim, K, num_trials, observed_tissue_model, observed_opt, tissue_index_list):
    # Here the training process starts
    train_loss, train_corr, train_spear_corr = np.zeros((num_updates, K)), np.zeros((num_updates, K)), np.zeros(
        (num_updates, K))
    test_loss, test_corr, test_spear_corr = np.zeros((num_updates, K)), np.zeros((num_updates, K)), np.zeros(
        (num_updates, K))
    ...
    for epoch in range(num_updates):
        zero_test_loss, zero_test_corr, test_prediction, test_true_label = zero_shot_test(zero_test_data_list,
                                                                                          feature_dim, layer, hidden,
                                                                                          observed_tissue_model)
        # print('0 Few shot', epoch, 'meta training:', '-1', '-1', zero_test_loss, zero_test_corr)

        epoch_folder = os.path.join(output_folder, 'epochs_' + str(epoch))
        ...
        for k in range(1, K + 1):
            ...
            tissue_train_loss, tissue_test_loss, tissue_train_corr, tissue_test_corr, = np.zeros(
                (num_trials,)), np.zeros(
            ...
            for i in range(num_trials):
                ...
                tissue_train_loss[i], tissue_train_corr[i], tissue_test_loss[i], tissue_test_corr[
                    i], test_prediction, test_true_label = unseen_tissue_learn(unseen_train_loader_list[i][k - 1],
                                                                               ...
                                                                               feature_dim,
                                                                               observed_tissue_model, hidden, inner_lr,
                                                                               num_inner_updates, observed_tissue_model)
        ...
        meta_update(observed_test_loader, grads, observed_tissue_model, observed_opt)
